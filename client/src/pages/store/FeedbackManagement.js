...
const FeedbackManagement = () => {
  ...
  const fetchFeedback = async () => {
    // Return early if there's no user context
    if (!user) {
      setLoading(false);
  ...
  const calculateStats = (feedbackData) => {
    if (!feedbackData || feedbackData.length === 0) {
      ...
      setStats({
        totalFeedback: 0,
        averageRatings: { overall: 0, service: 0, product: 0, cleanliness: 0, value: 0 }
      });
      return;
    }
    ...
    setStats({
      ...
      averageRatings: {
        ...
        product: (totals.product / count).toFixed(1),
  ...
  const renderStars = (rating) => {
    ...
    return Array(5).fill(0).map((_, i) => (
      <StarIconSolid
        key={i}
        className={`w-5 h-5 ${i < numericRating ? 'text-yellow-400' : 'text-gray-300'}`}
      />
    ));
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">View and analyze customer feedback and ratings.</p>
        </div>
        <button
          onClick={fetchFeedback}
          aria-label="Refresh feedback"
          className="inline-flex items-center justify-center sm:justify-start px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
          <ArrowPathIcon className="h-4 w-4 mr-2" />
          Refresh
        </button>
      ...
      {/* Stats Overview - More responsive grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
        <div className="bg-white p-5 rounded-lg shadow">
          <h3 className="text-base font-medium text-gray-500">Total Feedback</h3>
          <p className="mt-1 text-3xl font-bold text-gray-900">{stats.totalFeedback}</p>
        </div>
        
        {Object.entries(stats.averageRatings).map(([key, value]) => (
          <div key={key} className="bg-white p-5 rounded-lg shadow">
            <h3 className="text-base font-medium text-gray-500 capitalize">Avg. {key}</h3>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        {feedback.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            <ChatBubbleLeftRightIcon className="mx-auto h-12 w-12 text-gray-400" />
            <p className="mt-2 text-base">No feedback has been submitted yet.</p>
          </div>
        ) : (
          <ul className="divide-y divide-gray-200">
            {feedback.map((item) => (
              <li key={item.id} className="p-4 sm:p-6 hover:bg-gray-50">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    {/* Responsive header for rating and date */}
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                      ...
                      <div className="text-sm text-gray-500 flex items-center flex-shrink-0">
                        <ClockIcon className="inline-block h-4 w-4 mr-1.5" />
                        {formatDate(item.created_at)}
                      </div>
                    </div>
                    ...
                    {/* Responsive grid for detailed ratings */}
                    <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-3 text-sm">
                      {item.service_rating > 0 && (
                        <div>
                          <p className="text-gray-500">Service</p>
                          <div className="flex">{renderStars(item.service_rating)}</div>
                        </div>
                      )}
                      {item.product_rating > 0 && (
                        <div>
                          <p className="text-gray-500">Products</p>
                          <div className="flex">{renderStars(item.product_rating)}</div>
                        </div>
                      )}
                      {item.cleanliness_rating > 0 && (
                        <div>
                          <p className="text-gray-500">Cleanliness</p>
                          <div className="flex">{renderStars(item.cleanliness_rating)}</div>
                        </div>
                      )}
                      {item.value_rating > 0 && (
                        <div>
                          <p className="text-gray-500">Value</p>
                          <div className="flex">{renderStars(item.value_rating)}</div>
                        </div>
                      )}
                    </div>
                  </div>
